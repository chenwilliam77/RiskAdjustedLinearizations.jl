...
\end{align*}
Note that the forward-looking portion is not linear yet due to the $1 - \exp(\eta_{\beta, t + 1})$ term.
hence the stochastic discount factor becomes
\begin{align*}
  m_{t, t + 1} & = \underbrace{\eta_{\beta, t} + \log(\beta) - \overline{\beta}_t + \gamma c_t - (1 - \gamma) \ell_t - (\psi - \gamma) ce_t - \gamma \mu_a}_{\xi} \\
               &\quad + \underbrace{\overline{\beta}_{t + 1} - \gamma c_{t + 1} + (1 - \gamma) \ell_{t + 1} + (\psi - \gamma) v_{t + 1} - \gamma a_{t + 1}}_{\text{forward}}.
\end{align*}
...
\end{align*}
Thus, the primary problem in deriving the ccgf of a mixture model for the disaster risk is finding $C_1(s)$ and $C_2(s)$ for the size of the disaster shock. If $-\eta_{k, t} \sim \text{Gamma}(j_t, \sigma_k)$, then
\begin{align*}
