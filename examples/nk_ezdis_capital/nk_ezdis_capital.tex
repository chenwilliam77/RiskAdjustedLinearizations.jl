...
\begin{align}
  ...
  & \quad + \lambda_t \left(W_t L_t + (R_{k, t} + (1 - \delta + R_{q, t})Q_t)\exp(\eta_{k, t})K_{t - 1} + R_{t - 1} \frac{B_{t - 1}}{P_t} + F_t + T_t\right)\\
...
\begin{align*}
  ...
  \frac{\partial V_t}{\partial K_{t - 1}} & = \lambda_t(R_{k, t} + (1 - \delta + R_{q, t})Q_t)\exp(\eta_{k, t}).
\end{align*}
...
\begin{align*}
  \frac{\partial V_{t + 1}}{\partial K_t} = \lambda_{t + 1}(R_{k, t + 1} + (1 - \delta + R_{q, t + 1}) Q_{t + 1})\exp(\eta_{k, t + 1}).
\end{align*}
Substitute this expression and other quantities derived previously into the fourth first-order condition.
\begin{align*}
  0 & = V_t^{\psi}\exp(\eta_{\beta, t})\beta \calC\calE_t^{\gamma - \psi}\E_t\left[V_{t + 1}^{-\gamma}\lambda_{t + 1}(R_{k, t + 1} + (1 - \delta + R_{q, t + 1}) Q_{t + 1})\exp(\eta_{k, t + 1})\right] - \lambda_tQ_t.
\end{align*}
Divide by $\lambda_t$ and plug in $\lambda_{t + 1} / \lambda_t$.
\begin{align*}
  0 & = V_t^{\psi}\exp(\eta_{\beta, t})\beta \calC\calE_t^{\gamma - \psi}\\
    &\quad\times\E_t\left[\frac{V_{t + 1}^{\psi - \gamma} (1 - \exp(\eta_{\beta, t + 1})\beta)C_{t + 1}^{-\psi}\calL(L_{t + 1})^{1 - \psi}}{V_t^{\psi}(1 - \exp(\eta_{\beta, t})\beta)C_t^{-\psi} \calL(L_t)^{1 - \psi}}(R_{k, t + 1} + (1 - \delta + R_{q, t + 1}) Q_{t + 1})\exp(\eta_{k, t + 1})\right] - Q_t\\
...
\begin{align}
  ...
  Q_t & = \E_t\left[M_{t, t + 1} \left(R_{k, t + 1} + (1 - \delta + R_{q, t + 1})Q_{t + 1}\right)\exp(\eta_{k, t + 1})\right].
\end{align}
...
\begin{align}
  \frac{P_t(j)}{P_t}Y_t(j) - MC_t Y_t(j) - \kappa_y \exp(\eta_{a, t}).
\end{align}
...
\begin{align}
  \begin{split}
  &\max_{P_t(j)} \E_t\sum_{s = 0}^\infty\theta^sM_{t, t + s}\left(\frac{P_t(j)}{P_{t + s}}\left(\frac{P_t(j)}{P_{t + s}}\right)^{-\epsilon}Y_{t + s} - MC_{t + s}\left(\frac{P_t(j)}{P_{t + s}}\right)^{-\epsilon}Y_{t + s} - \kappa_y\exp(\eta_{a, t})\right),
  \end{split}
\end{align}
...
\end{align}
After production, capital producers return the initial investment of capital $\hat{K}_t$ to their owners and pay profits to households in proportion to the invested capital, hence the return from financing capital production is
\begin{align*}
  R_{q, t} & = \Phi\left(\frac{X_t}{\exp(\eta_{k, t})K_{t - 1}}\right) - \frac{X_t}{Q_t \exp(\eta_{k, t})K_{t- 1}}.
\end{align*}
From (\ref{eq:tobins q}), this expression can be re-written as
\begin{align}\label{eq:Rq defn}
  R_{q, t} & = \Phi\left(\frac{X_t}{\exp(\eta_{k, t})K_{t - 1}}\right) - \Phi'\left(\frac{X_t}{\exp(\eta_{k, t})K_{t - 1}}\right)\frac{X_t}{\exp(\eta_{k, t})K_{t - 1}}.
\end{align}
Although capital producers return the initial investment of capital, a fraction $\delta$ depreciates, so the evolution of the aggregate capital stock is
\begin{align}
...
\int_0^1( \hat{K}_t^\alpha (L_t\exp(\eta_{a, t}))^{1 - \alpha} - \kappa_y \exp(\eta_{a, t}))\, dj & = \int_0^1\left(\frac{P_t(j)}{P_t}\right)^{-\epsilon} Y_t\, dj\\
  \hat{K}_t^\alpha (\exp(\eta_{a, t})L_t)^{1 - \alpha} - \kappa_y\exp(\eta_{a, t}) & = Y_t \int_0^1\left(\frac{P_t(j)}{P_t}\right)^{-\epsilon} \, dj = \Delta_t Y_t.
\end{align*}
...
\begin{align}\label{eq:output market clearing}
  ...
  Y_t & = \frac{(\exp(\eta_{k, t}) K_{t - 1})^{\alpha}(\exp(\eta_{a, t})L_t)^{1 - \alpha} - \kappa_y \exp(\eta_{a, t})}{\Delta_t}.
\end{align}
...
\begin{align}
  ...
  R_{q, t} & = \Phi\left(\frac{\tilde{X}_t}{\exp(\eta_{k, t})\tilde{K}_{t - 1}}\right) - \Phi'\left(\frac{\tilde{X}_t}{\exp(\eta_{k, t})\tilde{K}_{t - 1}}\right)\frac{\tilde{X}_t}{\exp(\eta_{k, t})\tilde{K}_{t - 1}},\\
  ...
  \tilde{Y}_t & = \frac{(\exp(\eta_{k, t}) \tilde{K}_{t - 1})^{\alpha}(L_t)^{1 - \alpha} - \kappa_y}{\Delta_t}.
\end{align}
...
\begin{align*}
  Y & = \frac{(\exp(\eta_k)K)^\alpha L^{1 - \alpha} - \kappa_y}{\Delta}.
\end{align*}
...
\begin{align*}
  C + \overline{X} \exp(\eta_k)K = \frac{(\exp(\eta_k)K)^\alpha L^{1 - \alpha} - \kappa_y}{\Delta}.
\end{align*}
...
\begin{align*}
  ...
  C + \overline{X} \exp(\eta_k) K & = \left(\left(\frac{\alpha}{1 - \alpha}\right)^{\alpha} \left(\frac{W}{R_K}\right)^\alpha L - \kappa_y\right)\Delta^{ - 1}.
\end{align*}
...
\begin{align*}
  0 & = \log(\exp(\alpha(\eta_{k, t} + k_{t - 1}) + (1 - \alpha) l_t) - \kappa_y) - \log(\Delta_t) - y_t.
\end{align*}
...
(\ref{eq:denominator recursion eqm stat}). To ensure accuracy of the risk-adjusted linearization, I derive $N$-period
...
The jump variables are $y_t$, $c_t$, $l_t$, $w_t$, $r_t$, $\pi_t$, $q_t$, $x_t$, $r_{k, t}$, $r_{q, t}$, $mc_t$, $s_{1, t}$, $s_{2, t}$, and $\log(\Delta_t)$.
The state variables are $k_{t - 1}$, $\log(\Delta_{t - 1})$, $r_{t - 1}$, $y_{t - 1}$, and the autoregressive processes. The equations defining the evolution of the lags $v_{t - 1}$, $r_{t - 1}$, and $y_{t - 1}$ are obtained by the formula $z_{(t - 1) + 1} = z_t$.
