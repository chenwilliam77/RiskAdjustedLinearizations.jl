...
\end{align}
where $C_t$ is consumption, $B_{t - 1}$ nominal bonds, $X_t$ investment, $W_t$ the real wage, $L_t$ labor, $R_{K, t}$ the gross real rental rate on capital,
...
written as
\begin{align*}
  \max_{P_t(j)} \E_t\sum_{s = 0}^\infty(\beta \theta)^s\frac{\prod_{q = 1}^s \exp(\eta_{\beta, t + q})}{\exp(\eta_{\beta, t})}\frac{u'(C_{t + s})}{u'(C_t)}\left(\frac{P_t(j)}{P_{t + s}}\left(\frac{P_t(j)}{P_{t + s}}\right)^{-\epsilon}Y_{t + s} - MC_{t + s}\left(\frac{P_t(j)}{P_{t + s}}\right)^{-\epsilon}Y_{t + s}\right),
\end{align*}
where I have imposed that intermediate output equals demand. The first-order condition is
\begin{align*}
0 & =  (1 - \epsilon)P_t(j)^{-\epsilon}\E_t\sum_{s = 0}^\infty (\beta\theta)^s \frac{\prod_{q = 1}^s\exp(\eta_{\beta, t + q})}{\exp(\eta_{\beta, t})}\frac{u'(C_{t + s})}{u'(C_t)}(P_{t + s})^{-(1 - \epsilon)}Y_{t + s}\\
  &\quad + \epsilon P_t(j)^{-\epsilon - 1}\E_t\sum_{s = 0}^\infty (\beta\theta)^s \frac{\prod_{q = 1}^s \exp(\eta_{\beta, t + q})}{\exp(\eta_{\beta, t})}\frac{u'(C_{t + s})}{u'(C_t)} MC_{t + s}P_{t + s}^{\epsilon}Y_{t + s}
\end{align*}
...
\begin{align*}
  P_t(j) & = \frac{\epsilon}{\epsilon - 1}\frac{\E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{q = 1}^s \exp(\eta_{\beta, t + q})u'(C_{t + s}) MC_{t + s}P_{t + s}^{\epsilon}Y_{t + s}}{\E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{q = 1}^s \exp(\eta_{\beta, t + q})u'(C_{t + s})P_{t + s}^{\epsilon - 1}Y_{t + s}}\\
         & = \frac{\epsilon}{\epsilon - 1}\frac{\E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{q = 1}^s \exp(\eta_{\beta, t + q})u'(C_{t + s}) MC_{t + s}P_t^\epsilon\left(\prod_{u = 1}^s\Pi_{t + u}\right)^{\epsilon}Y_{t + s}}{\E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{q = 1}^s \exp(\eta_{\beta, t + q})u'(C_{t + s})P_t^{\epsilon - 1}\left(\prod_{u = 1}^s\Pi_{t + u}\right)^{\epsilon - 1}Y_{t + s}}\\
  \frac{P_t(j)}{P_t} & = \frac{\epsilon}{\epsilon - 1}\frac{\E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{q = 1}^s \exp(\eta_{\beta, t + q})u'(C_{t + s}) MC_{t + s}\left(\prod_{u = 1}^s\Pi_{t + u}\right)^{\epsilon}Y_{t + s}}{\E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{q = 1}^s \exp(\eta_{\beta, t + q})u'(C_{t + s})\left(\prod_{u = 1}^s\Pi_{t + u}\right)^{\epsilon - 1}Y_{t + s}}.
\end{align*}
This expression gives the optimal (real) reset price $P_t^* \equiv P_t(j) / P_t $ (note that the RHS does not depend on $j$).
Define
\begin{align*}
  S_{1, t} & = \E_t\sum_{s = 0}^\infty  (\beta\theta)^s \prod_{u = 1}^s \exp(\eta_{\beta, t + u})u'(C_{t + s}) MC_{t + s}Y_{t + s}\left( \prod_{u = 1}^s\Pi_{t + s}\right)^{\epsilon},\\
  S_{2, t}  & = \E_t\sum_{s = 0}^\infty (\beta\theta)^s \prod_{u = 1}^s \exp(\eta_{\beta, t + u})u'(C_{t + s})Y_{t + s}\left(\prod_{u = 1}^s\Pi_{t + s}\right)^{\epsilon - 1}.
\end{align*}
...
\begin{align*}
  \tilde{S}_2 & = Y + \theta\beta \Pi^{\epsilon - 1}\tilde{S}_2 \Rightarrow \tilde{S}_2 = \frac{Y}{1 - \theta\beta \Pi^{\epsilon - 1}}.
\end{align*}
Thus,
\begin{align*}
