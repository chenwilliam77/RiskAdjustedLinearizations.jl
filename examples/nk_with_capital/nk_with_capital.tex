...
\begin{align}
  ...
  \tilde{S}_{1, t} & = MC_t Y_t + \theta\beta\E_t[M_{t + 1} \Pi_{t + 1}^\epsilon \tilde{S}_{1, t + 1}],\\
  \label{eq:denominator recursion}
  \tilde{S}_{2, t} & =  Y_t + \theta\beta\E_t[M_{t + 1} \Pi_{t + 1}^{\epsilon - 1} \tilde{S}_{2, t + 1}].
\end{align}
...
\begin{align*}
  V_t^p & = \int_0^{1 - \phi}\left(P_t^*\right)^{ - \epsilon}\, dj + \int_{1 - \phi}^1 \left(\frac{P_{t - 1}(j)}{P_t}\right)^{ - \epsilon}\, dj\\
...
\begin{align}\label{eq:price dispersion evol}
  V_t^p & = \Pi_t^{\epsilon}((1 - \phi) (P_t^* \Pi_t)^{\epsilon} + \phi v_{t - 1})
\end{align}
...
\begin{align*}
  1 & = \frac{\Pi_t^{1 - \epsilon}}{(1 - \phi)(P_t^*\Pi_t)^{1 - \epsilon} + \phi}\\
  0 & = \log\E_t\left[\exp\left((1 - \epsilon)\pi_t - \log((1 - \phi) \exp((1 - \epsilon)(p_t^* + \pi_t)) + \phi)\right)\right].
\end{align*}

Equation (\ref{eq:price dispersion evol eqm}) becomes
\begin{align*}
  1 & = \frac{V_t^p}{\Pi_t^\epsilon((1 - \phi) (P_t^*\Pi_t)^{1 - \epsilon} + \phi V_{t - 1}^p)}\\
  0 & = \log\E_t\left[\exp\left(v_t - \log((1 - \phi) \exp(\epsilon(p_t^* + \pi_t)) + \phi \exp(v_{t - 1}))\right)\right].
\end{align*}
