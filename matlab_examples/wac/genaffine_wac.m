...
% to the solution of Wachter (2006); the methodology is described in
...
rhos = 0.97;
xi = 0.011;
beta = exp(-.0147/freq+gamma*ga-0.5*gamma*(1-rhos-xi/gamma));
S = sqrt(gamma*sigmaa^2/(1-rhos-xi/gamma));

% Optional: provide an initial guess for the deterministic steady state
SS_x = [-log(beta*exp(-gamma*ga));log(beta*exp((1-gamma)*ga)/(1-beta*exp((1-gamma)*ga)));[1:N]'*log(beta*exp((1-gamma)*ga));[1:N]'*log(beta*exp((1-gamma)*ga))+log(beta*exp((1-gamma)*ga)/(1-beta*exp((1-gamma)*ga)));0];
